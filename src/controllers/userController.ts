  ...
  try {
    ...
    if (existingUser) {
      if (!existingUser.verified) {
        ...
        res.status(200).json({ message: "User already registered but not verified. A new OTP has been sent." });
        return;
      }
...
export const sendOtpResend = async (req: Request, res: Response): Promise<void> => {
  try {
